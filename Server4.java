		...
		//		final int windowSize = 5;
				...
				if(packetsRemaining >=5){
				    for (int i = 0; i < windowSize; i++) {
				            byte[] customData = new byte[1016];
         				    ...
         				    byte[] sendData = new byte[1024];
       					    ...
       					    CustomPacket customPacket = new CustomPacket(i, id, customData, sendPacket);        					   System.out.println("Sending packet, sequence #" + i);
				    ...
				    for (int i = 0; i < lastWindowSize; i++) {
							System.out.println("packets remaining: " + packetsRemaining);
					    ...
					    System.out.println("received ack for sequence # " + ackNum);
 						...
 						byte[] customData = new byte[numBytesLastPacket];
                                                ...
                                                DatagramPacket sendPacket = new DatagramPacket(sendData, sendData.length, ipAddress, portnumber);
                                                CustomPacket customPacket = new CustomPacket(i, id, customData, sendPacket, numBytesLastPacket+8);
                                                System.out.println("Sending packet, sequence #" + i);
                                                window[i]=customPacket;
                                                serverSocket.send(window[i].packet);
				...
				/*	byte[] ack = new byte[4];
					...
					int ackNum = bytesToInt(ack);	
				*/		
					//if necessary resend packets here
